...
def plotcors(data, deldata, oslon, oslat, sy, ey, level, col='bw',ll='0'):
    ...
    levels5=[-1, -0.001, 0.001, 1]

    if col == 'color':
        delfill = ax.contourf(oslon, oslat, deldata, 10, transform=ccrs.PlateCarree(), cmap=plt.cm.coolwarm)
    ...
    if col == 'bw':
        ...
        fill = ax.contour(oslon, oslat, data, 10, transform=ccrs.PlateCarree(), colors='k', linewidths=0.68)
        clevels = []
        filllevels = fill.levels
        for i in range(0,len(filllevels),2):
            clevels.append(filllevels[i])
        ax.clabel(fill,clevels, fontsize=10)
    ...
    plt.savefig('%s' % (level +'   '+ str(sy) + '--' + str(ey)))
    #添加网格线
